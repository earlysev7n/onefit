...
- `GeneticAlgorithm` (`lib/algorithms/genetic_algorithm.dart`) — **the meal generator** (per the capstone objective). `generatePlan({allIngredients, profile, cuisine})` evolves combinations of `MealIngredient`s (the USDA-seeded `ingredients` collection) into a 7-day `List<DayMealPlan>`, scored by a fitness function on the user's calorie/macro targets; filters by cuisine (`any`/`filipino`/`western`/`asian`) and dietary restrictions. Wired into the Plans → Meal tab's Generate / Generate All. Ingredients are loaded via `FirestoreService.getIngredients()` (in-memory cached; `FirestoreService.clearIngredientCache()` drops it after a re-seed) and seeded with `SeedData.seedIngredients()` (Profile screen button, gated by `kShowSeedTools`).
  ...
  - **Budget-aware meal completion (accounts for manually-logged food).** The Meal tab no longer skips meals that already have logged food, and generation no longer ignores what's eaten. Two pure additions drive this: `GeneticAlgorithm.mealBudgets({goal, loggedCalsByMeal})` (static) returns the per-meal **additions** budget — each meal's natural deficit `max(0, ratio·goal − logged_m)` scaled by `remainingDay / Σdeficits`, so the budgets sum to `goal − Σlogged` (logged + generated ≈ goal; all-zero when the day is already at/over goal, never overloading). `completeMeal({allIngredients, profile, mealType, calorieBudget, presentCategories, cuisine})` returns the complementary **additions only** (NOT the logged items): it builds slot plans by `MealIngredient.category` (breakfast protein/grain/fruit; lunch/dinner protein/grain/vegetable/fat; snack = one snack-appropriate item) and **drops any slot whose food group is in `presentCategories`** so a logged protein gets a carb + fruit, not more protein (all-present → one low-density vegetable item), then `scaleToCalories(budget)`. Budgets ≤ 30 kcal or an empty pool → empty meal. The Meal tab computes `presentCategories` from logged `FoodItem`s via `_inferCategory` (name hints + the GA's own macro thresholds, since FoodItems carry no category). `_generateMeal` fills one meal to `ratio·goal − loggedCals(meal)`; `_generateAll` uses `mealBudgets` across all four. Because completion returns additions-only, **the save path is unchanged** — "Log Additions" persists only the new `ai_generated` items, so already-logged food is never double-saved. Asserted in `test/meal_completion_test.dart`. (`generatePlan`'s 7-day evolutionary path is retained for tests but no longer called by the Meal tab — completion replaced the cached-day flow.)
  ...
  - **Daily calorie carry-forward (rule-based, provider-level, for today only).** `DailyCarry.dailyAdjustedGoal({baseGoal, yesterdayLogged, carryFraction:0.5})` (`lib/algorithms/daily_carry.dart`, pure) nudges today's calorie goal by yesterday's over/under-eating: `adjusted = baseGoal − (yesterdayLogged − baseGoal)·0.5`, clamped to `[baseGoal·0.75, baseGoal·1.25]`; returns `baseGoal` when `yesterdayLogged` is null. `ProfileProvider` (`lib/providers/profile_provider.dart`) computes this at `load()`/`refresh()` time via `_computeDailyGoal(uid)` and exposes `dailyEffectiveGoal` (int) + `effectiveMacroGoals` (Map). **All display screens** (Home, Nutrition, Plans Meal tab, Progress, Profile) read `profileProvider.dailyEffectiveGoal` instead of `profile.calorieGoal` — so the adjusted number is the visible, app-wide goal for today. `ProgressProvider` receives it via `loadAll(effectiveGoal:)` for the today-ring and chart; however, `ProgressProvider.calorieAdherence` (the weekly %) still uses `profile.calorieGoal` as the denominator so it stays consistent with the weekly `AdaptationEngine`. **This is deliberately isolated from the weekly system:** it never writes `calorieAdjustment`, is never persisted (provider-level transient state), and `UserProfile.calorieGoal` / `AdaptationEngine.compute()` / `WeeklySummary` are all untouched. Asserted in `test/daily_carry_test.dart`.
