...
| Exercise preference | **Drag-to-rank list (2 items) + "?" info** | Reorderable Compound / Isolation ranking — the **only** selection-relevant type preference. The order (top = preferred) is a small tie-breaker: preferred type +2, other +1 (`_exerciseTypePoints`), deliberately far below the +50 focus / −25/−15 balance anchors. No point badges — the order alone is the signal. Stored inside the full 5-item `UserProfile.goalPriorities` (reconstructed on save for back-compat); `_scoreExercise` reads only the Compound-vs-Isolation order. Heavy Lift / High Rep / Full Body are **no longer** ranked here (Heavy/High moved to Training Focus; Full Body is workout coverage). |
