...
| Time per session | Chip group 30/45/60/90 min + "?" info | Drives **how many** exercises per day (`_fitExerciseCount`) only — it does **not** affect which exercises are picked (that's Exercise Priority + Experience). |
...
| Physical limitations | **Affected-area chips → per-area sheet** | 10 areas from `kPhysicalLimitations` (Asthma, High Blood Pressure, Shoulder/Knee/Lower Back/Wrist/Elbow/Hip/Ankle/Neck Pain). Tapping a chip selects the area (auto-block on) and opens a bottom sheet — "Exercises to Avoid (Optional)" with a **Commonly Affected** checklist first and a **Show All Exercises** expander for the rest; checks feed `UserProfile.avoidedMovements`, "Remove limitation" deselects the area. The generator hard-excludes both layers (area auto-block + opted-in movements) via `exerciseBlockedByLimitations(e, physicalLimitations, avoidedMovements)` in `isEligibleForUser`. A muted disclaimer ("not medical advice") sits under the chips. Empty by default → no filtering. |
