...
class ExerciseDBService {
  ...
  static const _gifCdnBase =
      'https://cdn.jsdelivr.net/gh/$_gifRepo@$_gifRepoRef';
  ...
  Future<List<Exercise>> getExercises() async {
    final meta = await _db.collection(_cacheCollection).doc(_metaDoc).get();
  ...
  Future<void> _buildGifIndex() async {
    ...
    try {
      ...
      if (resp.statusCode != 200) {
        if (kDebugMode)
          debugPrint('ExerciseDBService: GitHub GIF index ${resp.statusCode}');
        return;
      }
      ...
      if (kDebugMode)
        debugPrint('ExerciseDBService: _buildGifIndex failed – $e');
    }
  }
  ...
  Future<void> _saveToFirestore(List<Exercise> exercises) async {
    ...
    for (int i = 0; i < staleDocs.length; i += chunkSize) {
      final chunk = staleDocs.sublist(
        i,
        (i + chunkSize).clamp(0, staleDocs.length),
      );
    ...
    for (int i = 0; i < exercises.length; i += chunkSize) {
      final chunk = exercises.sublist(
        i,
        (i + chunkSize).clamp(0, exercises.length),
      );
      final batch = _db.batch();
  ...
  Future<List<Exercise>> _fetchFromApi() async {
    ...
    try {
      ...
      for (int page = 0; page < maxPages; page++) {
        final uri = Uri.parse(
          '$_base/exercises',
        ).replace(queryParameters: cursor == null ? null : {'after': cursor});
        ...
        exercises.addAll(
          rawList.cast<Map<String, dynamic>>().map(_mapToExercise),
        );
  ...
  Exercise _mapToExercise(Map<String, dynamic> raw) {
    ...
    final equipments = List<String>.from(
      raw['equipments'] ??
          (raw['equipment'] is String ? [raw['equipment']] : []),
    );
  ...
  List<String> _normalizeEquipment(List<String> raw) {
    ...
    final normalized = raw
        .map((e) => map[e.toLowerCase()] ?? _titleCase(e))
        .toList();
  ...
  List<String> _inferGoals(List<String> bodyParts, String name) {
    ...
    final isJumpMove =
        ...
        RegExp(
          r'\b(box|broad|depth|split|vertical|long)\s+jump\b',
        ).hasMatch(name);
